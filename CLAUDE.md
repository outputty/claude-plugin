...
3. `/build <n>` takes the ticket to draft PRs (a stack when large), docs last. The user reviews and types "merge".
...
- `.claude/roadmap.md` - Next, Later and Killed, one line each; the only doc that lists open tickets; architecture carries `pending #<n>` markers until they ship.
