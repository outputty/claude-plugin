...
Why each open ticket is worth building, and now. Status lives on the GitHub board, not here. `/plan` adds a paragraph when it files a ticket; the docs layer moves it under Shipped. `init` drafts Next from open issues and TODOs, Shipped from the last releases, and Killed from reverts and abandoned branches.
