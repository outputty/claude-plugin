...
- **A question with options carries the whole decision.** Restate the problem in one line above the options. Each option is a short summary plus a real example of the finished thing: for code, the highest-level example that still shows the difference (a call site, a config line, a CLI line), never the internals. Below the example, one line on what it changes and what it costs.
