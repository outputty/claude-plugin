---
name: retro
description: Turns this session's corrections and rework into rules, each written to the one file that loads it next time. Runs at the end of every planning session (after breakdown files) and inside every fix-issue build before its PR opens, and on "retro", "lesson", "what should we remember".
---
