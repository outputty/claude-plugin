...
2. **Draft the ticket the answer makes possible**: `add_task` `{ project, id: <slug>-<stamp>, title,
   brief, contract, scope, discovered_from: <this spike's id> }`. The `issue-authoring` skill owns
   its shape, and the bar it clears is *dispatchable*: a cold, unattended child can build it from
   the ticket alone.
