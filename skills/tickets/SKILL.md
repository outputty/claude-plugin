...
```bash
gh issue list --label ready --state open --json number,title,labels,assignees,createdAt
```
...
```text
/goal ticket #42 is built: <Done when case 1>; <case 2>; every layer is an open draft PR in one stack with the docs layer last; or stop after 60 turns
```
