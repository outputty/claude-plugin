...
For each selected domain, write `.claude/skills/<domain>/SKILL.md` from `${CLAUDE_PLUGIN_ROOT}/templates/SKILL.md`: the description says when a ticket needs it, the body is self-contained for quick judgements (every line actionable on its own, a few hundred lines at most, generic to the domain), and the explanations, worked cases and source files move under `.claude/skills/<domain>/references/`, each pointed at from the line it explains. The source files of a dropped domain are left where they are and named in the PR body. A domain skill loads itself when `/plan` or `/build` meets a ticket in its domain.
