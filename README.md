...
3. **BUILD** *(hands-off)* — a dynamic workflow: per task, execute then review the scoped diff; each layer's passed tasks commit serially. Retry once, escalate on a double failure.
