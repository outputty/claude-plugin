...
- **A hands-off build loop** drains a dependency-ordered task graph (see [Task tracking](#task-tracking)),
  **one builder + one QA per layer**: a Sonnet builder builds all of a layer's tasks **test-first** (a
  ...
  pushes, and posts a terse PR comment. The model is **tiered by role** — Sonnet builds, Sonnet-at-xhigh
  reviews, Haiku does the mechanical commit, and Opus runs the final whole-build master QA. A layer still
